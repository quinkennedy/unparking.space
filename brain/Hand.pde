public class Hand{
  ...
  int touchedAt;
  int x, y;
  color baseColor = color(155, 0, 155);
  color touchedColor = color(218, 165, 32);
  int period = 3000;
  int waitUntil;
  color fadeFrom;
  ...
  public Hand(PApplet parent, int x, int y, int offset){
  ...
  public color currBase(){
    ...
    return lerpColor(0, baseColor, progress);
  }
  ...
  public void draw(){
    ...
    if (touched){
      ...
      c = lerpColor(fadeFrom, touchedColor, progress);
    } else {
      c = currBase();
    }
    fill(c);
